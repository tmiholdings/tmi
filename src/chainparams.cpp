...
// Copyright (c) 2018 The TMIcoin Private developers
// Copyright (c) 2017-2018 The TMIcoin Core developers
    ...
    const std::string& strGenesisBlockSignatureBase64)
{
    ...
    genesis.sig.resize(TMI_SIGN_BYTES * genesis.iVerificationLength);
...
public:
	CMainParams() {
        strNetworkID = "main";
        // tmi : 반감기값, nSubsidyHalvingInterval 값으로 설정된 블록마다 보상이 절반으로 줄어듬
		...
		//consensus.TMIHeight = std::numeric_limits<int>::max(); //491407; // Around 10/25/2017 12:00 UTC
        //consensus.TMIPremineWindow = 8000;
        consensus.TMIZawyLWMAHeight = std::numeric_limits<int>::max(); // Not activated on mainnet
        consensus.TMIEquihashForkHeight = std::numeric_limits<int>::max(); // Not activated on mainnet
        //consensus.TMIPremineEnforceWhitelist = true;
        ...
        // tmi : Digishield난이도 조정 알고리즘의 매개 변수입니다. (30블록마다 난이도가 변경)
        ...
        consensus.TMIMaxFutureBlockTime = 12 * 10 * 60; // 120 mins
        
		//tmi : test, 약 1시간마다 갱신, 블럭은 3초 
...
public:
    CTestNetParams() {
        ...
        //consensus.TMIHeight = std::numeric_limits<int>::max();//1;
        consensus.TMIZawyLWMAHeight = -1; // Activated on testnet
        consensus.TMIEquihashForkHeight = std::numeric_limits<int>::max(); // Not activated on testnet
        //consensus.TMIPremineWindow = 50;
        //consensus.TMIPremineEnforceWhitelist = false;
        ...
        consensus.nZawyLwmaAveragingWindow = 45;
        consensus.nZawyLwmaAjustedWeight = 13632;
        consensus.TMIMaxFutureBlockTime = 7 * 10 * 60; // 70 mins
...
public:
    CRegTestParams() {
        ...
        //consensus.TMIHeight = std::numeric_limits<int>::max(); //2000;
        consensus.TMIZawyLWMAHeight = -1; // Activated on regtest
        consensus.TMIEquihashForkHeight = 2001;
        //consensus.TMIPremineWindow = 10;
        //consensus.TMIPremineEnforceWhitelist = false;
        consensus.powLimit = uint256S("ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
        consensus.powLimitStart = uint256S("ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
        consensus.powLimitLegacy = uint256S("ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
...
//class TMIcoinAddressChainParam : public CMainParams
//{
//public:
//    TMIcoinAddressChainParam()
//    {
...
//static TMIcoinAddressChainParam chainParamsForAddressConversion;
...
//const CChainParams &TMIcoinAddressFormatParams()
//{
...
bool CChainParams::IsPremineAddressScript(const CScript& scriptPubKey, uint32_t height) const {
    ...
    assert(0/*(uint32_t)consensus.TMIHeight <= height*/ &&
           1/*height < (uint32_t)(consensus.TMIHeight + consensus.TMIPremineWindow)*/);
    //int block = height - consensus.TMIHeight;
    //int num_unlocked = consensus.TMIPremineWindow * 40 / 100;  // 40% unlocked.
    //int num_locked = consensus.TMIPremineWindow - num_unlocked;  // 60% time-locked.
    ...
    //    int lock_time = consensus.TMIHeight + stage_lock_time * (1 + stage);
