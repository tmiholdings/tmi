...
#if TMI_SIGN_BYTES != crypto_sign_BYTES
    error "TMI_SIGN_BYTES != crypto_sign_BYTES"
#endif
...
bool CBlockHeaderSignatureValidation::verifyBlockHeader_(const CBlockHeader* pBlockHeader) const
{
    ...
    LogPrintfd("if(getCountOfSignaturesRequiredToPassValidation()[%d] != iVerificationLength[%d] || ((iVerificationLength * TMI_SIGN_BYTES)[%d] != sig.size()[%d])) ",
        getCountOfSignaturesRequiredToPassValidation(), iVerificationLength, iVerificationLength * TMI_SIGN_BYTES, sig.size());
    //유효성 검사를 통과하는 데 필요한 서명의 수량이 다른가?
    if(getCountOfSignaturesRequiredToPassValidation() != iVerificationLength
        || ((iVerificationLength * TMI_SIGN_BYTES) != sig.size()))
        return false;

    const uint256 hash = pBlockHeader->GetHash();

    uint16_t iPassCount = 0;
    for (size_t i = 0; i < m_vPublicKey.size() && (((size_t)(iPassCount + 1) * TMI_SIGN_BYTES) <= sig.size()); i++)
    {
        ...
        if (crypto_sign_PUBLICKEYBYTES == m_vPublicKey[id].size()) {
            int iOK = crypto_sign_verify_detached(sig.data() + (iPassCount * TMI_SIGN_BYTES), hash.begin(), hash.size(), m_vPublicKey[id].data());
