#!/bin/bash
# Copyright (c) 2013 The TMIcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete TMIcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no TMIcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old TMIcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected TMIcoin 0.7 datadir."
    ;;
  3)
    echo "Detected TMIcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected TMIcoin 0.8 datadir."
    ;;
esac
