...
###   tmicoincore.org and tmicoin.org and compares them.
...
WORKINGDIR="/tmp/tmicoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://tmicoincore.org"
HOST2="https://tmicoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="tmicoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'tmicoin-',
...
if [ $? -ne 0 ]; then
   echo "tmicoin.org failed to provide signature file, but tmicoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "tmicoin.org and tmicoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the TMIcoin Core binary release signing key installed?"
   fi
