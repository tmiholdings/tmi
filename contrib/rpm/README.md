...
The RPM spec file provided here is for TMIcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://tmicoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[TMIcoin git repository](https://github.com/tmi/tmi) but are not part of
...
Currently only `Source10` is used, for the example `tmicoincore.conf` file.
...
in the tmicoin git repository. At present only an SVG version of the TMIcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the tmicoin
...
needed when 0.12.1 is released, a proper fix is already in the TMIcoin git
master branch.
...
idea to build TMIcoin Core against a static version of that library compiled
...
The `tmicoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `tmicoin-server` package. At this point, most RPM based
...
The `tmicoin-server` package should have SELinux support. How to properly do
...
TMIcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/tmicoin/). He has been
packaging TMIcoin for Fedora at least since 2012.
...
4. I renamed the `tmicoin` package that contains the Qt GUI to `tmicoin-core` as
that appears to be how the general population refers to it, in contrast to
`tmicoin-xt` or `tmicoin-classic`. I wanted to make sure the general population
