...
class ChainstateWriteCrashTest(TMIcoinTestFramework):
    def __init__(self):
        super().__init__()
        self.num_nodes = 4
    ...
    def restart_node(self, node_index, expected_tip):
        ...
        while time.time() - time_start < 60:
            try:
                ...
                # If tmicoind exits, then try again.  wait_for_node_exit()
                # should raise an exception if tmicoind doesn't exit.
                self.wait_for_node_exit(node_index, timeout=10)
            self.crashed_on_restart += 1
            time.sleep(1)

        # If we got here, tmicoind isn't coming back up on restart.  Could be a
        # bug in tmicoind, or we've gotten unlucky with our dbcrash ratio --
