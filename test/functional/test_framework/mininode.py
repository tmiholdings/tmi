...
# Copyright (c) 2010-2016 The TMIcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""TMIcoin P2P network half-a-node.

This python code was modified from ArtForz' public domain  half-a-node, as
found in the mini-node branch of http://github.com/jgarzik/pynode.

NodeConn: an object which manages p2p connectivity to a tmicoin node
...
COIN = 100000000 # 1 TMI in satoshis
...
TMI_REGTEST_HARDFORK_HEIGHT = 2000
...
# Objects that map to tmicoind objects, which can be serialized/deserialized
...
class CTransaction(object):
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in tmicoind
            if (flags != 0):
                self.vin = deser_vector(f, CTxIn)
...
class CBlockHeader(object):
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            if self.nHeight < TMI_REGTEST_HARDFORK_HEIGHT:
...
class msg_headers(object):
    ...
    def deserialize(self, f):
        # comment in tmicoind indicates these should be deserialized as blocks
...
class NodeConnCB(object):
    """Callback and helper functions for P2P connection to a tmicoind node.
...
class NodeConn(asyncore.dispatcher):
    ...
    def __init__(self, dstaddr, dstport, rpc, callback, net="regtest", services=NODE_NETWORK, send_version=True):
        ...
        logger.info('Connecting to TMIcoin Node: %s:%d' % (self.dstaddr, self.dstport))
