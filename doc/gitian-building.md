...
*Setup instructions for a Gitian build of TMIcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the TMIcoin
...
- [Building TMIcoin Core](#building-tmicoin-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-tmicoin-host/README.md).
...
Clone the git repositories for TMICOIN and Gitian.
...
in the tmicoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building TMIcoin Core
----------------

To build TMIcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the tmicoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/TMICOIN/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/tmi/tmi
...
For example:
```bash
URL=https://github.com/akx20000a/TMICOIN.git
COMMIT=d5522ed66a924a2d505fe45ed2487e741c6c0917
./bin/gbuild --commit TMICOIN=${COMMIT} --url TMICOIN=${URL} ../TMICOIN/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit TMICOIN=${COMMIT} --url TMICOIN=${URL} ../TMICOIN/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit TMICOIN=${COMMIT} --url TMICOIN=${URL} ../TMICOIN/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the tmicoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../tmicoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/TMICOIN
SIGPATH=/some/root/path/tmicoincore-detached-sigs

./bin/gbuild --url TMICOIN=${BTCPATH},signature=${SIGPATH} ../TMICOIN/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/tmicoin-core-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/tmicoin-core-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/tmicoin-core-osx-build.assert
```
...
[TMICOIN/gitian.sigs](https://github.com/tmi/gitian.sigs/) repository, or if that's not possible create a pull
request.
